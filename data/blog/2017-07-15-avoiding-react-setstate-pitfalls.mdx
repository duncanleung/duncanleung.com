...
date: '2017-07-15'
tags: ['react']
summary: 'A guide to avoiding common pitfalls when using setState in React, including asynchronous updates, batching, and state management patterns.'
images: ['/static/thumbnails/react.png']
draft: false
---

`setState()` can be easily misunderstood in React, and considering that managing component state is a fundamental aspect of React, I wanted to understand the common pitfalls and solutions around using `setState()`.
...
`setState()` is **asynchronous**. Consequently, the values of `this.state` should not be used for calculating the next state.
