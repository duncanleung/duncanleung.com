...
summary: 'A deep dive into React pure render optimization and method binding patterns, exploring different approaches for improving component performance.'
...
The motivation behind keeping the `render()` method pure is to [avoid issues when using `shouldComponentUpdate()`](https://facebook.github.io/react/docs/react-component.html#shouldcomponentupdate), and `React.PureComponent`, which work by [performing shallow comparison checks](https://facebook.github.io/react/docs/react-api.html#react.purecomponent).

The guidelines of a pure `render()` are:
...
The following two non-pure patterns, using `.bind` and arrow functions, **return a new function every time the Component re-renders**. This will cause problems when trying to utilize `shouldComponentUpdate()` and `React.PureComponent`.
...
ES6 Class syntax is the current standard for writing React (the React docs have also switched over to use ES6 Class syntax). However, when using the traditional syntax of `React.createClass`, the `this` context of methods in the Component is actually automatically bound.
...
<iframe
  src="https://codesandbox.io/embed/oY825BRxK?view=editor"
  style={{ width: '100%', height: '1600px', border: 0, borderRadius: '4px', overflow: 'hidden' }}
