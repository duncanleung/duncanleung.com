...
date: '2017-08-14'
tags: ['react']
summary: 'Understand and resolve React synthetic event warnings in setState, with best practices for event handling and state updates.'
...
- [React `setState()`](https://facebook.github.io/react/docs/react-component.html#setstate) is asynchronous.
- [React `SyntheticEvent`](https://facebook.github.io/react/docs/events.html) cannot be accessed asynchronously.
...
An example of the code I was attempting to run was: _(Open up the console and try to run the demo in **preview view** to see the error)_

<iframe
  src="https://codesandbox.io/embed/KYP4KQzl?view=editor"
...
It turns out [React has its own event system](https://facebook.github.io/react/docs/events.html) for event handling, using `SyntheticEvent`.
...
The console warning I encountered occurs because React re-uses the `SyntheticEvent` object for performance reasons, by pooling the synthetic events together. Thus, all the properties on `event.target` are nullified after an event callback is invoked.
