...
date: '2019-12-03'
tags: ['testing', 'jest']
summary: 'A detailed guide to testing Formik form validation schemas using Jest and React Testing Library, covering validation rules, error handling, and test patterns.'
...
I needed to test a form `PersonalInfoForm.tsx` that included a validation schema for field validation, but was running into a warning when I was programmatically changing input values with Jest and react-testing-library.
...
```js {49-56}import React from 'react';
...
```tsx {45-77,107}import React, { useCallback } from "react";
...
<a
  href="https://github.com/jaredpalmer/formik/blob/master/src/Formik.tsx#L516-L546"
  target="_blank"
>
  In the Formik source code
</a>
, `setValues` and `setFieldValues` both use the hook `useEventCallback`. Formik validation is async,
and so `useEventCallback` returns a `Promise`.
...
```ts {5}const setValues = useEventCallback((values: Values) => {
...
```js {7,14}describe("Form Interaction", async () => {
  //...
  //...
