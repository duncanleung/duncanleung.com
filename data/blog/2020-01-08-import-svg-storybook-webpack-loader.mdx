...
summary: 'A detailed guide to setting up Webpack loaders in Storybook for importing and using SVG files as React components.'
...
> The `@svgr/webpack` loader rule needs to occur before the other loaders such as the `file-loader` rule so that `@svgr/webpack` takes precedence.
