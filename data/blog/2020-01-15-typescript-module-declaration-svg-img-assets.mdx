...
summary: 'A guide to create TypeScript module declarations for SVG and image assets, including webpack configuration and type definitions.'
...
## Create an index.d.ts Module Declaration File
...
Moving the `index.d.ts` for `.svg` files to its own `./src/@types/assets` subfolder allowed TypeScript to correctly recognize the `.svg` module declarations.
