...
```js {6-7}// The example settings below tell ESLint to error when single-quotes are used, and when semi-colons are missing.
...
```js {4}// The example settings below tell ESLint to use the eslint:recommended base configuration and the eslint-config-airbnb shared configuration.
...
```js {5}// This file adds some React specific settings. Not using React? Use base.js instead.
...
```js {5,9,17-18}// ... imports and setup
