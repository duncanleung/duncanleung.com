---
title: 'Setting Up ESLint: Understanding Config Files and Plugins'
date: '2020-01-28'
tags: ['eslint']
summary: 'A guide to set up ESLint, explaining the differences between ESLint config files and plugins, with detailed configuration examples and best practices.'
...
```js {4}
// The example settings below tell ESLint to use the eslint:recommended base configuration and the eslint-config-airbnb shared configuration.
...
```js
// The example settings below tell ESLint to use the eslint-plugin-react linting rules, and manually configures two rules.
...
Each environment brings with it a certain set of predefined global variables, and prevents ESLint from throwing a `no-undef` rule error when referencing an expected global variable such as, `window`, or `document`.
...
```js
// The example settings below specify ESLint to enable support for ECMAScript 2018 and JSX.
...
Look for the relevant **eslint-plugin-\*** to add additional linting rules that ESLint should check for in a **React + TypeScript project**.
...
It's common for **eslint-plugin-\*** authors to bundle a **recommended** rule configuration inside a plugin to use as a starting point.

I usually start with these **recommended** configs, and manually configure any overrides in the `rules` property later.

_Note that the bundled configuration itself does not enable any of the plugin's rules by default. These recommended configs should be treated as a standalone **eslint-config-\***._
...
[@typescript-eslint/eslint-plugin](https://github.com/typescript-eslint/typescript-eslint/blob/master/docs/getting-started/linting/README.md#getting-started---linting-your-typescript-codebase) comes with a bundled **recommended** base config.
...
[eslint-plugin-react](https://github.com/yannickcr/eslint-plugin-react#configuration) comes with a bundled **recommended** base config.
...
✨ Using **eslint-config-airbnb-typescript** is also nice because the installation docs specify a <a href="https://github.com/iamturns/eslint-config-airbnb-typescript#i-use-eslint-config-airbnb-with-react-support" target="_blank" rel="noopener noreferrer">very simple minimum setup</a>:
...
Now you'll understand why there are some tutorials and blogs that have certain configs set, while other articles omit those properties.
