...
- **Errors throw** in the `async` style. Wrap multi-step handlers in `try`/`catch` for centralized error handling. But handler `try`/`catch` only catches code errors - permission denials, missing env vars, and event-source misconfigurations need [integration and end-to-end tests against a deployed stage](/blog/serverless-testing-strategy-integration-over-unit).
