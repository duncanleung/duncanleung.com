...
Lambda duration is rounded up to the nearest 100ms - the billing granularity in effect at this date. The numbers below are picked as multiples of 100ms so the rounding stays invisible and the math reads cleanly. (AWS moved to 1ms billing in December 2020; the relative cost of sync chaining is unchanged under either rule.)
...
export const handler = async (event: CreateOrderEvent) => {
  const validationResponse = await lambda
    .invoke({
      FunctionName: process.env.VALIDATE_ORDER_FN_NAME!,
      InvocationType: 'RequestResponse',
      Payload: JSON.stringify({ order: event.order }),
    })
    .promise()
...
export const handler = async (event: APIGatewayEvent) => {
  ...
  const response = await lambda
    .invoke({
      ...
      InvocationType: 'RequestResponse',
      Payload: JSON.stringify({ id: event.queryStringParameters.id }),
    })
    .promise()

  return {
    statusCode: 200,
    body: response.Payload as string,
  }
}
```
...
export const handler = async (event: CreateOrderEvent) => {
  ...
  const validation = JSON.parse(validationResponse.Payload as string)
...
- Sync direct invoke is fine when both functions are owned by the same team and live in the same service. The internal contract is yours to manage - but it is still a wire-level contract, so test it [end-to-end against a deployed stage](/blog/serverless-testing-strategy-integration-over-unit) rather than with mocks of `lambda.invoke`.
