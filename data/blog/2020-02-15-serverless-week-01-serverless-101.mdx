...
date: '2020-02-15'
tags: ['serverless', 'aws-lambda', 'aws-api-gateway', 'aws-iam']
summary: 'DRAFT: Serverless Notes'
...
- see what services were accessed by, and I'll stress this, the dedicated deployer user
  (screenshot in Production Ready Serverless: 1. Intro > Managing permissions)
...
- There are companies that have this limit raised up to tens of thousands of concurrent lambda executions.
...
- Looking at `Throttles` graph is useful, we don't want our functions to be throttled. Most likely exhausted available concurrent executions
...
- Pseudo parameters
- step functions
- split stacks (for larger stacks - help with 200 resources limit)
- dotenv
- IAM Roles per Func

## Destinations

- for async invocations and streaming based traffic - processing events from kinesis or DynamoDB streams
...
- Put into shared module in same project
