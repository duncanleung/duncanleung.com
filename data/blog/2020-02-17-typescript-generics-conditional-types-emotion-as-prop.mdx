...
date: '2020-02-17'
tags: ['typescript', 'emotion']
summary: 'A guide to implement TypeScript generics and conditional types for type-safe Emotion components with dynamic as props, including practical patterns and examples.'
...
```typescript {12-13}import React from "react";
...
```typescript {1}<Button as={Link} to="/login">
  Log In
</Button>
```
...
```typescript {5-6}//... taken from the above example
...
When rendering the component as a native HTML anchor element `<a>`, we need to include the attribute, `href`:
...
[Conditional Types](https://www.typescriptlang.org/docs/handbook/advanced-types.html#conditional-types) allows you to select one of two possible types based on a type-relationship condition.
...
```typescript
type GetRenderComponentProps<T, P = {}> = T extends
...
The return type of the `<Button />` component also depends on the Generic type `T`.

```typescript {4,7,8}type RenderComponent<P = {}> = React.ComponentType<React.PropsWithChildren<P>> | typeof Link | "a";
...
This intersection now correctly resolves to the type depending on the type of element, `E`, passed into the `as` prop.

```typescript {3}type Props<E extends RenderComponent, P = {}> = {
  as?: E;
...
```typescript {10,12-14,16,21}import React from "react";
import styled from "@emotion/styled";
import { Link } from "gatsby";
