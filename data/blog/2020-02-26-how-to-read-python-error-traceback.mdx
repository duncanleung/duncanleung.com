...
- **For resource cleanup that must always run** (closing files, releasing locks), prefer `with` over `try` / `finally`. See [Python Context Managers](/blog/python-context-managers-with-statement) for the protocol behind it.
