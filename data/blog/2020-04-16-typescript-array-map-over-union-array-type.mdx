...
date: '2020-04-16'
tags: ['typescript']
summary: 'A guide to properly handling array map operations with union types in TypeScript, including type inference and type safety considerations.'
...
```typescript {7,13}import React from "react";
...
Apparently, for union types, members which are functions will also be typed as union types, so the type of `Array.prototype.map` is not callable.
