...
date: '2020-04-27'
tags: ['graphql', 'gatsby', 'i18n']
summary: 'A detailed guide to implementing dynamic locale selection in Gatsby using Contentful, GraphQL variables, and React Context for internationalized content.'
images: ['/static/thumbnails/graphql.png']
draft: false
---
...
Contentful provides a nice headless CMS platform that handles internationalization, and [gatsby-source-contentful](https://github.com/gatsbyjs/gatsby/tree/master/packages/gatsby-source-contentful) allows us to access Contentful data during build time.
...
```typescript {8}// Page content shape from Contentful
export type Page = {
...
```typescript {5,11,25-26,30}import React from "react";
...
```typescript {9-16}type QueryData = {
  contentfulPage: {
    edges: {
      node: Page;
    }[];
...
To do this, we modify our GraphQL query by creating the `(\$intlLocale: String)` variable definition after the query operation name `ContentfulQuery`, which declares `$intlLocale` as a variable accepted by the query.

(Similar to an argument definition for a function in a typed language, a GraphQL variable definition lists all of the variables, prefixed by `\$`, followed by their type.)

Now we can query the `contentfulPage` by the `node_locale` that matches the `$intlLocale` variable from React-Intl.

```typescript {2,3}export const query = graphql`
query ContentfulQuery($intlLocale: String) {
...
```javascript {4,20}exports.onCreatePage = ({ page, actions }) => {
...
```typescript {5-9,19,23}import React from "react";
import { PageRendererProps, graphql } from "gatsby";
