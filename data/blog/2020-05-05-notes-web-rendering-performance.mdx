...
- 1 second / 60fps = 16.6ms to do JavaScript execution work
...
- JavaScript
  JavaScript is used to handle work that results in visual changes (animate, sorting a data set, adding DOM elements)
...
a "composite only" change does not require layout or paint:
...
Computed style calculations occur when changes happen to the DOM through:
...
- Avoiding layout thrashing (making style changes).
...
- changing the size of an element
  ...
  - The size of its parent element
