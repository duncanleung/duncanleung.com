...
date: '2020-05-10'
tags: ['aws', 'serverless']
summary: 'A detailed guide to using Serverless Framework pseudo parameters for referencing Lambda functions and other AWS resources in your serverless applications.'
...
A handy trick to reference an AWS Lambda API endpoint is to use [serverless-pseudo-parameters](https://www.serverless.com/plugins/serverless-pseudo-parameters/) and [Serverless variables](https://www.serverless.com/framework/docs/providers/aws/guide/variables/) to create the endpoint URL that can be passed to lambda function through an environment variable.
...
[Pseudo parameters](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/pseudo-parameter-reference.html) are parameters that are predefined by AWS CloudFormation that return AWS resource IDs.
...
Serverless generates a `cloudformation-template-update-stack.json` CloudFormation template from our `serverless.yml`.
...
This CloudFormation template specifies `ApiGatewayRestApi` as an API Gateway REST API resource that contains a collection of API Gateway resources.
...
`${self:provider.stage}` self-references the provider stage variable property in our serverless.yml.
