...
date: '2020-05-28'
tags: ['aws', 'serverless', 'debugging']
summary: 'A step-by-step guide to setting up and using breakpoints for local debugging of AWS Lambda functions, improving serverless development workflow.'
...
In this example we're invoking the function named `index` with the flag `--function`. The function name `index` is specified by the `serverless.yml`.
