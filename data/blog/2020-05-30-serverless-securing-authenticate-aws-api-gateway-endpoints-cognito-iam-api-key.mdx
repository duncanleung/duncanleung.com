...
summary: 'A guide to implement authentication and authorization in AWS API Gateway, covering Cognito User Pools, IAM roles, and API keys with practical examples.'
...
<img
  className="avatar"
  src="/static/images/post-images/2020-05-29-lambda-get-api.png"
  alt=""
/>
...
<img
  className="avatar"
  src="/static/images/post-images/2020-05-29-lambda-post-api.png"
  alt=""
/>
...
<img
  className="avatar"
  src="/static/images/post-images/2020-05-29-lambda-fanout-api.png"
  alt=""
/>
...
```yaml {3-37,39-47,49-57}resources:
  Resources:
    CognitoUserPool:
...
```yaml {12-14,18-52,54-62,64-72,74-83}provider:
  name: aws
  runtime: nodejs12.x
...
In order to invoke a Lambda that is secured with an IAM authorizer, we'll need to sign and prepare our requests using [AWS Signature Version 4](https://docs.aws.amazon.com/general/latest/gr/sigv4_signing.html).

Add the [aws4](https://github.com/mhart/aws4) NPM package.
...
```js {5,10-13,15,18}const http = require("axios");
...
You can set up a [general Usage Plan](https://www.serverless.com/framework/docs/providers/aws/events/apigateway/#setting-api-keys-for-your-rest-api) that all API keys will use, but I like to have specific Usage Plan categories and assign keys to each specific category.
