...
summary: 'Automatically format and lint code with Git hooks using Husky, Prettier, ESLint, precise-commits and lint-staged for pre-commit and pre-push workflows.'
images: ['/static/thumbnails/prettier.png']
draft: false
---

# Automatically Format and Lint Code with Git Hooks

I wanted to set up automatic code formatting and linting before a `git commit` (pre-commit) or before a `git push` (pre-push) that was independent of each team member's IDE-specific setup.

Having a gated `git push` also prevents lint-failing code from being pushed up to our team's remote branches.
