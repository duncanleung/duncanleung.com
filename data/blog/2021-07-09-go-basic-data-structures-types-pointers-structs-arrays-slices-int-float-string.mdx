...
date: '2021-07-09'
tags: ['backend', 'go']
summary: 'Understanding Go data structures and their memory layout including integers, floats, arrays, pointers, structs, strings, and slices.'
...
- `Point` is represented in memory as two adjacent `int`s
...
- The length is 2, so `y[0]` and `y[1]` are the only valid indexes
- The capacity is 4, so `y[0:4]` is a valid slice expression
...
- `new(Point)` returns a pointer to the zeroed memory of a type `Point`
...
- `new(Rect1)` returns a pointer to the zeroed memory of a type `Rect1`
...
- `new(Rect2)` returns a pointer to the zeroed memory of a type `Rect2`
