...
summary: 'Fixing TypeScript ESLint errors when using SVGs with Next.js 11 by creating custom module declarations and configuring TypeScript properly.'
...
```typescript {5,9}import { default as Logo } from '~/public/logo.svg';
...
```typescript {7}import { default as XIcon } from '~/public/icons/x.svg';
...
Next.js 11 <a href="https://github.com/vercel/next.js/pull/26485" target="_blank">introduced its own image import types</a> to prevent conflicts with existing image handling set-ups.
...
```typescript {3}  /// <reference types="next" />
...
```typescript {7}declare module "*.svg" {
  /**
...
### Create a custom next-env.d.ts to exclude image-types/global
...
```typescript {23-26}  type StaticImageData = {
    src: string;
    height: number;
    width: number;
    placeholder?: string;
