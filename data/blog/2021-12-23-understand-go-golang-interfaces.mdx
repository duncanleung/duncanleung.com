...
date: '2021-12-23'
tags: ['backend', 'go']
summary: 'A guide to understand Go interfaces through a power socket and electrical device analogy, covering duck typing and interface implementation.'
...
```go {2,3}// The type of electrical devices
type Blender struct{}
...
Each electrical device should also be able to draw power from the power socket.
...
```go {6-7}// The instances of these devices
var blender Blender
var microwave Microwave
...
```go {2}// The type of the power socket
...
```go {3}// Socket has a max power capacity
type PowerSocket struct {
	powerCapacity int
}
...
```go {2}// PowerSockets can plug in any Blender
...
```go {2}// Plug in a Microwave to draw power from the PowerSocket
...
```go {2,4}// Appliance represents an electrical device that can draw power.
...
```go {2,6,7}// Plug in any Appliance device to the power socket
...
```go {3}// Appliance represents an electrical device that can draw power.
type Appliance interface {
	Draw(power int)
  ^^^^^^^^^^^^^^
}
...
To implement an interface, a struct needs to implement the function signature of the interface.
...
```go {3,10,17}// Appliance represents an electrical device that can draw power.
type Appliance interface {
	Draw(power int)
  ^^^^^^^^^^^^^^
}
...
```go {2,22,26}// Plug in any Appliance device to the power socket
func (s *PowerSocket) PlugIn(appliance Appliance) error {
                                       ^^^^^^^^^
