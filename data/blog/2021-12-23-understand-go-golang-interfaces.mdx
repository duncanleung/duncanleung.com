...
- A common "plug shape" (Go interface) to describe how each electrical device can plug into a power socket.
...
Let's add a property `powerCapacity` to the `PowerSocket` struct to describe the maximum power output that an outlet can support.
