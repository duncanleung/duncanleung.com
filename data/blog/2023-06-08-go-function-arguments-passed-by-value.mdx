...
date: '2023-06-08'
tags: ['backend', 'go']
summary: 'How Go passes function arguments by value and the implications for different data types: integers, pointers, slices, strings, and structs.'
...
In practice, this means that inside the function, any changes made to this copy of the value do not affect the original variable.
...
The "copy of the pointer" points to the same memory location as the original value, and any changes made to the data in the memory location that the pointer points to will affect the original data.
...
In addition to allowing modification of the original value in a calling function, passing a pointer is also used to indicate that a function parameter is optional.
...
Here, `modifySlice` can change an element of the original slice. Although a copy of the slice header is passed in, it still points to the same underlying array.
...
This makes passing strings efficient, as the actual sequence of bytes that make up the string is not copied.
