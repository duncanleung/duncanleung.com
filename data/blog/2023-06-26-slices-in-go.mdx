...
date: '2023-06-26'
tags: ['backend', 'go']
summary: 'Understanding Go slices, their underlying array structure, creation and modification mechanics, and the implications when passing slices to functions.'
...
func main() {
	...
	s2 := []int{1, 2, 3, 4, 5, 6}
	fmt.Printf("Value of s2: %#v\n", s2) // Value of s2: []int{1, 2, 3, 4, 5, 6}
}
```
...
In Go, although function arguments are "pass by copy", we need to pay attention when passing slices to functions because it is actually a copy of the "slice header" that is being passed.
