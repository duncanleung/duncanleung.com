...
date: '2023-06-30'
tags: ['backend', 'go']
summary: 'Understanding the Go idiom of accepting interfaces and returning concrete types to write flexible, reusable, and maintainable code.'
...
When the same concrete type is genuinely consumed by many call sites with overlapping subsets of methods - a shared singleton wired through DI - declaring per-consumer interfaces in each consumer's package starts to duplicate. The refinement is to declare role interfaces at the provider and let each consumer narrow at its field. See [Producer, Service, ProducerService: Interface Segregation for Shared Singletons in Go](/blog/go-interface-segregation-shared-singletons).
