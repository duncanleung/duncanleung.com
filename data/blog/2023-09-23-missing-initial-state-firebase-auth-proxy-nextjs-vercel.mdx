---
title: 'Proxy Firebase Auth with Next.js on Vercel'
date: '2023-09-23'
tags: ['firebase', 'authentication']
summary: 'Solving Firebase Auth redirect issues on Vercel by implementing a proxy solution to handle SAML SSO authentication and resolve missing initial state errors.'
images: ['/static/thumbnails/firebase.png']
draft: false
---

## Problem

I was having issues implementing SAML single sign-on (SSO) authentication with Firebase on our Next.js web app hosted on Vercel.
...
Modern web browsers have security features that block third-party storage access (aka block third-party cookies) to protect user privacy by stopping third-party content providers and advertisers from tracking users across websites.

(End users could manually disable security settings like "Prevent Cross-site Tracking", however that's not really a reasonable solution).
...
5. The iframe is closed and redirected back to the app
6. The app reads the auth result from the `authDomain`'s browser storage

On a Firebase Hosted app, the `authDomain` is the same domain as the hosting domain: `<projectId>.firebaseapp.com`. In this case, the browser has no issues with cross-origin browser storage access.
...
3. Use a Next.js rewrite to proxy any requests to `myapp.com/__/auth/*` to `<projectId>.firebaseapp.com/__/auth/*`
...
3. Next.js rewrite reverse-proxies the request
...
6. Next.js rewrite reverse proxies the request
