...
This replaces `pyenv`. `uv` downloads and manages standalone Python builds for you, so there is no need to compile from source, so the `zlib` / Xcode header issues from the old `pyenv` workflow disappear.
...
`uv` keeps the lockfile and environment in sync automatically on `add` / `remove` / `run`. You can also run them explicitly, which is useful in CI or after a fresh `git clone`.
